...
public class ArrayDeque61B<T> implements Deque61B<T>{
    ...
    private class ArrayIterator<T> implements Iterator<T> {
        ...
        public T next() {
            int returnNumber = wizPos;
            wizPos = Math.floorMod(wizPos + 1, capacity);
            return (T) get(returnNumber);
        }
    }
    ...
    public boolean equals(Object list) {
        ...
        if (list instanceof ArrayDeque61B<?>) {
            if (this.size() != ((ArrayDeque61B<?>) list).size()) { return false; }
            for (int i = 0; i < this.size(); i++) {
                if (!(this.get(i) == ((ArrayDeque61B<?>) list).get(i))) { return false; }
            }
            return true;
        }
        return false;
